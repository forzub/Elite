...
**Статус: `[~]` transport/process + reconnect-control + explicit authentication/admission baseline защищён automated/manual acceptance. M8E.2f закрыт; текущий короткий M8E.2g polish вводит server-known stable AccountHandle, strict/localized form validation, responsive short-window layout и dev auth reset. После него следующий открытый слой — M8E.3 durable authoritative universe persistence, включая password/recovery + account/player/ship ownership и изменяемое состояние мира.**
...
- `[x]` M8E.2 убрал implicit first-connect enrollment: unknown `SignIn` больше не получает свободный `PlayerId`; binding создаётся только явным `Register`, duplicate live session rejected typed `AlreadyActive`, а reconnect сохраняет server-owned identity/control rules. `[x]` M8E.2f закрыл WebView recovery race и неявный process-level `default` credential. `[~]` M8E.2g фиксирует stable AccountHandle/validation/reset/responsive UI. `[ ]` M8E.3 делает identity и mutable universe state durable across restart; client-selected `--player`/`--entity` authority shortcut запрещён.
...
- `[ ]` **UI platform / binary resource pack:** `EliteUiKit` + versioned `elite_ui.pak` для общих WebView components, HTML/CSS/JS/locales/icons/fonts. Сервисные и игровые WebView-страницы становятся тонкими compositions над общей библиотекой.
- `[ ]` **Bundled font coverage:** клиент поставляет собственные лицензируемые шрифты/fallbacks для поддерживаемых скриптов (Latin/Cyrillic/Chinese/Japanese) и проверяет glyph coverage; системные шрифты не являются гарантией.
- `[ ]` **Full registration form:** отдельный REGISTER view содержит DisplayName, AccountHandle, password/confirmation, recovery channel/code, locale и явные consent fields. DisplayName — Unicode; AccountHandle остаётся строгим ASCII login.
- `[ ]` **Remembered device + last account:** `ClientPreferencesStore` запоминает последний успешно вошедший AccountHandle на конкретном server endpoint, а Windows Credential Manager хранит только секретный remembered-device credential. Password в preferences не пишется.
...
- `[ ]` **ESC session menus:** multiplayer — Resume/Settings/Disconnect-to-main/Sign-out/Quit без паузы мира; local — Resume/New/Save/Load/Main/Quit, причём manual Save/Load разрешены только authoritative safe-save policy на базе/в safe zone.
...
3. `[ ]` **UI/auth platform pre-slice — ближайший короткий приоритет.** Общий `EliteUiKit + elite_ui.pak`, bundled font coverage, remembered last account/server и reusable registration/sign-in/error/modal components нужны до full password/recovery UI, чтобы следующие сервисные страницы не размножали CSS/JS/fonts.
