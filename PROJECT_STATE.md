...
- Клик/drag tactical overlay захватывает gesture до release и не должен одновременно запускать camera orbit/pan. Активный Hub синхронизируется с canonical map selection и снова включает `HUB`; выбор корабля/инфраструктуры снимает старый Hub/body/cube focus, а обычный выбор тела/куба снимает tactical-object focus.
- Длина local/global velocity arrows теперь кодирует модуль скорости логарифмически и ограничена прежним максимальным экранным размером; это сохраняет читаемость между маневровыми и глобальными скоростями.
