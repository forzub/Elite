...
function damageStateVariantsHtml(model,text){const rows=model?.rows||[];if(!rows.length)return `<span class="muted">${text.empty}</span>`;return rows.map(row=>`<div class="listRow ${row.selected?'selected':''}" data-damage-variant-node="${row.nodeIndex}"><span class="badge">${row.id}</span><span class="grow">${row.nodeId}${row.detached?' · DETACHED':''}</span>${row.transformOverride?'<span class="badge">T</span>':''}${row.physicsOverride?'<span class="badge">P</span>':''}</div>`).join('');}
