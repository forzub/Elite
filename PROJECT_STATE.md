...
- `[ ]` Stage M8C: добавить Asio TCP client/server transport adapters + connection accept/admission/disconnect lifecycle и localhost two-process smoke, не меняя `GameServer`/`ServerRuntime`.
...
2. `[~]` **Multiplayer session/player authority foundation** — Stage M1–M7 готовы; M8A зафиксировал portable/versioned control-plane wire protocol; M8B добавил extensible ordered data schema для snapshot/map response и schema-blind compression seam. Далее M8C подключает настоящий TCP process transport.
...
8. Stage M8B — **готов**: `SimulationSnapshot` + `MapResponse` проходят canonical ordered schema -> raw bytes -> schema-blind compression envelope -> framing; adding a field normally touches DTO + `WireDataSchema.h` + schema-version/test, но не TCP/compressor/ServerRunner. Текущий следующий шаг — **M8C: Asio TCP client/server adapters и реальный localhost/process smoke**. Только затем — field-level delta/baseline optimization и возврат к `Scheduled <-> Coarse <-> Prewarm <-> Active` materialization/collapse.
