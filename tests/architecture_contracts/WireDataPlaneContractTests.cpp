...
SimulationSnapshot makeSnapshot()
{
    ...
    navigationExecution.valid = true;
    navigationExecution.intentRevision = 9001u;
    navigationExecution.activeTargetRevision = 8999u;
    navigationExecution.idealLinearAccelerationDemandMapMps2 =
        glm::dvec3(1.0, 2.0, 3.0);
    navigationExecution.idealAngularAccelerationDemandMapRadPerSec2 =
        glm::dvec3(0.1, 0.2, 0.3);
    navigationExecution.executedLinearAccelerationDemandMapMps2 =
        glm::dvec3(0.9, 1.8, 2.7);
    navigationExecution.executedAngularAccelerationDemandMapRadPerSec2 =
        glm::dvec3(0.09, 0.18, 0.27);
    navigationExecution.emergency = true;
    navigationExecution.hazardUrgency01 = 0.8;
    navigationExecution.reactionBlocked = false;
    navigationExecution.decisionSampled = true;
    navigationExecution.queuedCommandApplied = true;
    navigationExecution.pendingCommandCount = 3u;
    ship.navigationExecution = navigationExecution;
...
void testSnapshotRoundTrip()
{
    ...
    const auto* decodedNavigationExecution =
        std::get_if<game::simulation::NavigationExecutionSnapshot>(
            &decoded.ships[0].navigationExecution
        );
    require(decodedNavigationExecution != nullptr,
        "navigation execution payload did not round-trip");
    require(decodedNavigationExecution->valid,
        "navigation execution validity did not round-trip");
    require(decodedNavigationExecution->intentRevision == 9001u,
        "navigation execution intent revision did not round-trip");
    require(decodedNavigationExecution->activeTargetRevision == 8999u,
        "navigation execution active-target revision did not round-trip");
    require(near(
            decodedNavigationExecution->
                executedLinearAccelerationDemandMapMps2.y,
            1.8),
        "navigation executed linear demand did not round-trip");
    require(near(
            decodedNavigationExecution->
                executedAngularAccelerationDemandMapRadPerSec2.z,
            0.27),
        "navigation executed angular demand did not round-trip");
    require(decodedNavigationExecution->emergency &&
            decodedNavigationExecution->decisionSampled &&
            decodedNavigationExecution->queuedCommandApplied,
        "navigation execution diagnostics did not round-trip");
    require(decodedNavigationExecution->pendingCommandCount == 3u,
...
void testSparseNavigationExecutionVariant()
{
    using game::network::wire::WireWriter;
    using game::network::wire::binary::encodeValue;

    std::variant<
        std::monostate,
        game::simulation::NavigationExecutionSnapshot
    > absent = std::monostate {};

    WireWriter absentWriter;
    require(
        encodeValue(absentWriter, absent),
        "sparse navigation execution monostate encode failed"
    );
    require(
        absentWriter.bytes().size() == 1u,
        "absent navigation execution must cost only one variant tag byte"
    );

    auto presentExecution = game::simulation::NavigationExecutionSnapshot {};
    presentExecution.valid = true;
    presentExecution.intentRevision = 1u;

    decltype(absent) present = presentExecution;
    WireWriter presentWriter;
    require(
        encodeValue(presentWriter, present),
        "present navigation execution encode failed"
    );
    require(
        presentWriter.bytes().size() > absentWriter.bytes().size(),
        "present navigation execution must carry payload beyond the variant tag"
    );
}
...
int main()
{
    ...
    testSparseNavigationExecutionVariant();
