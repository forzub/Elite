...
7. `[~]` **M8E.1 recovery gate (2026-08-16):** graphical two-client world/identity separation работает, а reconnect control-epoch fix и его regression/architecture tests уже зелёные. Ручной runtime retest пока нельзя считать доказательным: выяснилось, что acceptance harness и ручной запуск могут использовать разные физические `EliteServer.exe`, поэтому старый binary способен воспроизвести уже исправленный source-level defect.
8. `[ ]` **P0.5 — canonical build/runtime layout и cleanup:** для каждой постоянной сущности проекта должен существовать один канонический build/runtime path, который используется всегда. Не создавать долговечные каталоги вида `server2`, `new_server`, `ready_headless_server`, `final_server` и т. п. Зафиксировать один путь для ручного `EliteGame`, один для dedicated `EliteServer`; test harness либо обязан проверять эти же production-development binaries, либо использовать только явно test-only scratch directories с автоматической очисткой. Провести инвентаризацию `build/*`, удалить неиспользуемые/устаревшие build-каталоги и обновить scripts/docs так, чтобы невозможно было случайно протестировать один executable, а вручную запустить другой.
9. `[~]` После закрытия M8E.1/P0.5 — проверить Win32 `WM_NCLBUTTONDOWN` modal-loop stalls отдельно от networking. Native event pump устранил cross-process GLFW AV, но `DispatchMessageW(WM_NCLBUTTONDOWN/HTCAPTION)` всё ещё может удерживать UI thread на 0.1–2.5 s при манипуляции окном. Это не должно смешиваться с reconnect/reconciliation диагнозом.
10. `[ ]` Только после зелёного two-process control/reconnect gate и единого build provenance продолжать authorization/persistent-world backbone и durable ship registry; не строить следующий слой поверх нестабильного session-control lifecycle.
...
13. **Немедленный operational block — canonical build/runtime layout + cleanup.** Зафиксировать единственные постоянные пути клиентского и серверного build, убрать неиспользуемые дублирующие каталоги, развести canonical binaries и временный test scratch, затем повторить reconnect/two-client runtime acceptance именно на каноническом `EliteServer.exe`.
14. **После зелёного recovery gate — полноценная server-owned authorization/identity skeleton**, а не временный CLI-костыль выбора игрока: `AccountId -> PlayerId -> owned/current ShipInstanceId -> ServerSessionId -> ControlAuthority`, duplicate-login/reconnect policy и persistent storage boundary. После фиксации этого слоя начинаем реально населять вселенную и подключать `Scheduled <-> Coarse <-> Prewarm <-> Active` materialization/collapse к реестру всех кораблей.
