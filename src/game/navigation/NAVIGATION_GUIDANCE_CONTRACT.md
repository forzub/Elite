...
belongs to the `TrajectoryGenerator` stage.

### 6.1 Time-parameterized trajectory layer

`world::navigation::TrajectoryGenerator` consumes one accepted geometric path,
one canonical `NavigationVehicleProfile`, the vehicle velocity relative to the
planning frame, and optional path-speed constraints. It remains independent of
client/server ownership, rendering and production simulation. Its coordinates
are explicitly relative to `Trajectory.frameId`; a Hub trajectory is Hub-local,
not a list of world-space points frozen at the planning epoch.

Stage 5C deliberately stops treating the coarse route as a polyline whose
vertices need local rounding. `SmoothPathOptimizer` instead builds a family of
clamped cubic B-spline candidates over the same obstacle-topology route. The
broadest candidate is tried first; progressively denser support controls are
introduced only when the smooth curve cuts canonical Sphere/OBB/Capsule
geometry. Candidate scoring makes curvature and curvature variation orders of
magnitude more expensive than path length, so a long readable approach is
preferred to a short sharp one. The docking composer is likewise allowed to
place its outer alignment point kilometres beyond the shortest approach.

The accepted spline is adaptively tessellated by maximum segment length and
chord/sagitta error, revalidated against canonical obstacle geometry, then
parameterized with curvature lateral-speed ceilings and forward/backward
acceleration/braking passes. Safety remains absolute: if every global smooth
candidate is blocked, the system may fall back to the safe coarse topology, but
it may not hide an obstacle crossing behind presentation smoothing.

Docking keeps a semantic inward terminal axis and max-entry speed, but no longer
requires a stop merely to hand off from a locally rounded corner. The final
controls are collinear with the docking axis so the global spline converges
tangentially into the aperture.

A generated trajectory stores physical execution offset plus universe time,
frame-relative position, velocity, acceleration, orientation and angular-velocity
samples. `universeTimeScale` is explicit: acceleration/braking use gameplay
seconds while ephemeris/frame projection uses the scaled universe clock. Stage 5A uses angular
state for deterministic preview/diagnostics only; the future follower owns
closed-loop attitude feasibility. A non-zero initial velocity component across
the route is reported as `pathCaptureRequired` rather than being silently
rotated onto the path.

For a moving planning frame, predictive consumers resolve samples through the same
`NavigationWorldPredictor` at the relevant universe time. Manual docking uses a
separate dynamic spatial tunnel described below; it is rebuilt at the current
guidance epoch rather than pretending its gates are future prediction samples.

### 6.2 Dynamic manual docking tunnel

`world::navigation::GuidanceTunnelBuilder` converts the accepted Hub-local
trajectory into an array of equidistant desired vehicle poses for manual flight.
The route/trajectory remain immutable until the next explicit `CALCULATE ROUTE`;
only the tunnel is rebuilt from the actual ship pose and the live rotating dock.
This keeps manual correction responsive without allowing HUD presentation to
move the underlying collision-free route.

Every refresh treats the current rendered ship pose and the live docking pose as
new boundary conditions. Already-flown trajectory progress is discarded; no gate
behind the ship is republished. A fresh global B-spline is constructed from the
ship through still-relevant downstream supports into three collinear docking-axis
controls. Thus leaving the old centreline changes the complete remaining tunnel
instead of merely dragging a short near-end offset.

Gate attitude is generated by a rotation-minimizing/parallel-transport frame
along that live curve. The current ship up-vector seeds the frame and the
remaining twist required by the dock is distributed smoothly from the beginning
of the remaining corridor to the aperture. The rectangle itself therefore shows
both route tangent and gradual roll/pitch/yaw correction; no separate UP marker
is required.

All tunnel gates use one constant rectangle size for the complete tunnel. The
size is derived once from the physical docking opening; it does not broaden or
narrow with distance. Lateral/vertical tolerances are the remaining centre
window after authored dock clearance and actual vehicle width/height are removed.
Gate positions are resampled at equal physical arc-length spacing, independent of
trajectory timestamps. The production spacing is intentionally sparse enough to
read as a corridor rather than a wire cage, and cockpit alpha fades with distance. The first on-ship gate may be suppressed by cockpit
presentation to avoid a giant near-camera rectangle, but it remains part of the
shared guidance product.

`GuidanceTunnelBuilder` is renderer/client/server agnostic and is linked into both
execution owners. Today the HUD consumes it; a future follower may consume the
same desired poses without inventing a second attitude path.
