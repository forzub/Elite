...
function populateLanguageSelect(){for(const id of ['toolbarLanguage','settingsLanguage']){const sel=$(id);if(!sel)continue;const selected=state.locale;sel.innerHTML=state.localeOrder.map(l=>`<option value="${l}">${languageName(state.i18n?.languages||{},state.locale,l)}</option>`).join('');sel.value=state.localeOrder.includes(selected)?selected:state.locale;sel.setAttribute('aria-label',tr('model_editor.settings.language','Interface language'));}}
function applyLocale(locale=state.locale){state.locale=state.localeOrder.includes(locale)?locale:(state.i18n?.default_locale||'en');document.documentElement.lang=state.locale;document.title=tr('model_editor.document.title','Elite Model Asset Editor');applyDocumentTranslations(document);applyTooltips();populateLanguageSelect();uiChrome.setTranslator?.(tr);uiChrome.refresh?.();refreshDynamicUi();applyDocumentTranslations(document);return state.locale;}
...
return {tr,translateServerMessage,loadI18n,bindToolTipElement,attachDynamicToolTip,populateLanguageSelect,applyLocale,applyDocumentTranslations,acceptSettingsLocale,persistLocale,cycleLocale};
};
