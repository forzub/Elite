...
- Current admission temporarily assigns an existing unowned materialized ship (preferring the legacy player ship and excluding Hub Motion Lab actors). Persistent account/character/ship ownership will replace only this selector; transport/session authority does not depend on that temporary policy.
