...
struct ProgramRun
{
    ...
    double maximumReferencePositionErrorMeters = 0.0;
    double maximumSpeedMps = 0.0;
};

ProgramRun runProgram(
    Vehicle& v,
    const Program& program,
    double maximumExtraSeconds
)
{
    ProgramRun out;
    ...
    const double hardStop = endTime + maximumExtraSeconds;
    ...
    while (v.timeSeconds < hardStop - 1.0e-9)
    {
        ...
        out.maximumReferencePositionErrorMeters =
            std::max(
                out.maximumReferencePositionErrorMeters,
        ...
        const auto systemIntent = toSystemIntent(follower.intent);
        const double nextTime = v.timeSeconds + kDt;
        const auto bridgeResult =
            v.bridge.step(nextTime, kDt, systemIntent);
        ...
        {
            out.completed = true;
            out.completionTimeSeconds = v.timeSeconds;
            break;
        }
    }
