...
- `loading.html` is transient presentation, never a terminal error state. A failed remote attach navigates back to the multiplayer authorization form with the preserved server/client reason; local failure returns to main menu.
- `SessionHello` carries an opaque bearer token plus explicit `AuthenticationIntent::{SignIn, Register}`. `SignIn` is resolution-only: an unknown credential receives `SessionRejectReason::UnknownCredential` and cannot fall through into account creation. `Register` is the only first-contact operation allowed to create an in-memory `AccountId -> PlayerId` binding.
...
- Multiplayer WebUI exposes endpoint + local credential profile + explicit `SIGN IN` / `REGISTER` / `BACK`. Local credential profile names are local selectors only; they never become `PlayerId`/`EntityId`. Local -> Multiplayer enters this authorization form instead of reusing local gameplay identity as remote authority.
...
- M8E.2 intentionally does **not** claim durable account persistence or Internet-grade transport security. `AccountRegistry` is still RAM-only, so a fresh dedicated-server runtime requires explicit registration again; TLS/credential rotation/account lifecycle remain later work.
...
Current limitations are deliberate: `AccountId` + bearer-token digest validation + explicit registration/sign-in now exist, but `AccountRegistry` is still in-memory and bootstrap player records are not durable account/character storage. The next architecture stage is M8E.3: a server-owned durable account store preserving `credential digest -> AccountId -> PlayerId -> owned/current ShipInstanceId` across restart, followed by explicit resume/takeover policy and durable universe ship records. A client must never obtain authority by sending an arbitrary PlayerId/ShipInstanceId/EntityId.
