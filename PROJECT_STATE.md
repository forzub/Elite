...
- `[~]` **UI platform / binary resource pack:** базовый `ELITEUI1` pack + `UiResourcePack` runtime boundary готовы; следующий slice — reusable `EliteUiKit` components. `EliteUiKit` + versioned `elite_ui.pak` для общих WebView components, HTML/CSS/JS/locales/icons/fonts. Сервисные и игровые WebView-страницы становятся тонкими compositions над общей библиотекой.
...
3. `[~]` **UI/auth platform pre-slice — текущий приоритет.** Binary `elite_ui.pak`, pinned global Noto WebUI font set, central license registry и locale direction/script metadata готовы в первом slice. Следом — reusable `EliteUiKit` registration/sign-in/error/modal components + `ClientPreferencesStore` (last server/account), после чего full password/recovery/social shell строится уже на общей UI платформе.
