...
- `[~]` **Remembered device + last account:** `ClientPreferencesStore` уже запоминает последний успешно вошедший AccountHandle на конкретном server endpoint и preferred locale в non-secret atomic preferences JSON. Текущий bearer credential остаётся в Windows Credential Manager; password/полноценный remembered-device lifecycle появится в account-security slice и в preferences не пишется.
...
3. `[~]` **UI/auth platform — текущий приоритет.** Binary `elite_ui.pak`, pinned global Noto WebUI font set, central license registry и locale direction/script metadata готовы. Второй slice добавляет shared `elite_ui.js/css` component kit, explicit native/WebView navigation shell и `ClientPreferencesStore` (last successful server/account + locale). Следом — full Registration / Sign In / Recovery / Account / ESC-social shell уже на общей платформе.
