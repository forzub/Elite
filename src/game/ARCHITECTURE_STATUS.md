...
- `loading.html` is transient presentation, never a terminal error state. A failed remote attach must return to the multiplayer authorization form with the preserved server/client reason; local failure returns to main menu. M8E.2f fixes the WebView2 navigation race discovered by manual acceptance: `main_menu.html` emits `main_menu_ready`, and C++ applies the pending Multiplayer sub-view/error only after that new DOM is active.
