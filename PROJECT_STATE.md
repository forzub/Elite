...
- `[ ]` Stage M8D: подключить TCP adapters к process host/session lifecycle: `EliteServer --listen` + remote `EliteGame`/`RemoteGameSession`, server-owned admission/disconnect/reconnect и отдельный localhost process smoke. Нельзя зашивать player/entity selection в клиентский packet.
...
9. Stage M8C — **готов на transport boundary**: standalone Asio `TcpWireStream` переносит только `WireMessageKind + opaque payload`, typed adapters реализуют существующие transport interfaces, а localhost contract проверяет обе стороны реальным kernel TCP. Текущий следующий шаг — **M8D: `EliteServer` listener/admission + remote game session и two-process localhost smoke**. После фиксации process reconnect/baseline lifecycle — field-level delta/baseline optimization и возврат к `Scheduled <-> Coarse <-> Prewarm <-> Active` materialization/collapse.
