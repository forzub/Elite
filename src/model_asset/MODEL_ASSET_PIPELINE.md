...
The **offline editor** has a stronger canonical authoring boundary. `CanonicalMeshBuilder` uses topology-aware point identity, collapsed/duplicate face cleanup and libigl `split_nonmanifold`, then repairs only provable local winding inconsistencies. Coherent open/thin components keep the authored Blender winding exactly; only a fully closed orientable shell may be flipped as one component when signed volume proves it is inside-out. Editor-owned code then rebuilds normals/render vertices/edges while preserving UV/material/hard-normal seams. Real authored holes are never capped. libigl is editor-only; Embree orientation is not part of PREPARE.
...
PREPARE does not guess an outside for coherent open/thin authored parts. The editor may still store an explicit editor-only whole-mesh orientation override (`AUTO` or `FLIPPED`) keyed to the accepted source revision. `FLIPPED` is applied *after* deterministic canonical preparation by reversing triangle winding and matching vertex normals; the resulting `.elmesh` is the runtime authority. The override metadata stays in the working/production editor sidecar only so an unchanged source reimport can reproduce the decision. When the source fingerprint changes the override becomes stale and is not silently applied to the new geometry.
