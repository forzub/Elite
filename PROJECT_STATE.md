...
10. `[x]` **M8E.2b — explicit authentication/admission:** `SessionHello` несёт только opaque bearer credential + явный `SignIn/Register` intent; неизвестный credential на `SignIn` получает typed `UnknownCredential` и больше не enroll'ится автоматически. Только явный `Register` может создать `AccountId -> PlayerId` binding; duplicate live login получает typed `AlreadyActive`. Typed `SessionReject` проходит через loopback/TCP до `GameClient`, а rejected TCP получает bounded flush grace вместо мгновенного обрыва.
11. `[x]` **M8E.2c — multiplayer authorization UI:** multiplayer form содержит endpoint + local credential profile и отдельные `SIGN IN` / `REGISTER`. Переход Local -> Multiplayer всегда приходит в эту форму; локальная session не определяет remote account identity. Sign-in читает только существующий credential slot, register может явно создать новый slot. Server-owned `AccountId/PlayerId/ShipInstanceId/EntityId` клиент по-прежнему не выбирает.
...
14. `[ ]` **M8E.3 — durable account/character store:** текущий `AccountRegistry` всё ещё живёт только в RAM. Нужен storage boundary, переживающий restart сервера, чтобы зарегистрированный credential всегда возвращался к тому же `AccountId -> PlayerId -> ShipInstanceId` независимо от порядка последующих подключений. До этого fresh server требует явного REGISTER.
15. `[ ]` После durable identity профилировать оставшиеся короткие loading-screen stalls; Win32 `WM_NCLBUTTONDOWN/HTCAPTION` modal-loop stress держать отдельной UI-thread responsiveness задачей, если он ещё воспроизводится.

### Multiplayer / session authority foundation

**Статус: `[x]` transport/process + reconnect-control + explicit authentication/admission baseline защищён automated contracts. Один dedicated canonical `EliteServer` обслуживает два отдельных `EliteGame`; remote identity проходит через explicit SIGN IN/REGISTER, unknown credential и duplicate live login имеют typed rejection, а клиент никогда не выбирает gameplay authority IDs. Следующий открытый слой — durable account/character storage across server restart; loading responsiveness остаётся отдельной задачей.**
...
14. Stage M8E.2 explicit authentication/admission — **закрыт на protocol/UI boundary**: `SIGN IN` не создаёт account, `REGISTER` является явным first-contact operation, typed rejection доходит до WebUI, Local -> Multiplayer всегда проходит authorization form. Следующий рабочий блок — **M8E.3 durable account/character storage** (`credential digest -> AccountId -> PlayerId -> owned/current ShipInstanceId` across restart), после него — durable universe population/materialization.
