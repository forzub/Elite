...
- `[ ]` Stage M8E: два отдельных remote client processes одновременно на одном dedicated server + disconnect isolation; затем explicit reconnect/resume token/session handoff без client-selected EntityId.
