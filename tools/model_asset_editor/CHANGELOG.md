...
- Re-saving an already-current stage checkpoint is allowed and records a new monotonic `checkpointSequence`. REIMPORT/RELOAD/ordinary edits still do not move the saved timeline, RESTORE remains non-destructive, and saving an earlier stage still prunes every later checkpoint. Hard serialization/I/O failure remains the only reason a checkpoint cannot be recorded.
- Removed the heavy SURFACES side effects from explicit `ClosedVolume / ThinOneSided / ThinTwoSided / BreachedVolume` changes. The authoring edit remains geometry-scoped metadata; it no longer launches model preflight, serializes material usage for every triangle in the asset, retransmits geometry, or triggers a full browser `rebuildScene()`.
- Added a targeted `surface_metadata_patch` control-plane message. The browser updates only the affected GeometryDefinition metadata, cached preflight row and resident Three.js material sidedness. Cross-LOD application batches the small patches and performs no automatic topology analysis; `ANALYZE SURFACES` remains the explicit expensive audit.
- Added lightweight `wizard_state_patch` publication for checkpoint state changes so checkpointing itself no longer serializes the whole asset metadata or rebuilds the station scene. Asset v4, CanonicalMeshBuilder, libigl, Embree, LOD generation and geometry identity contracts are unchanged.

## 0.10.24 — linear checkpoint resume / binary geometry transport

- Checkpoint resume head is now selected by an explicit monotonic `checkpointSequence`, stored inside each stage-local `editor_state.json`. The sequence advances only after a successful `COMPLETE STAGE + CHECKPOINT`; tab changes, REIMPORT, RELOAD, RESTORE, ordinary edits and `wizard_state.json` rewrites cannot move the saved timeline.
- Existing schema-8/legacy checkpoints remain loadable. Until the first new sequenced checkpoint is saved, migration ordering prefers the stage-local `editor_state.json` timestamp (written only by explicit checkpoint SAVE) and falls back to the package timestamp only for checkpoints that predate stage-local editor state. Once any sequenced checkpoint exists, filesystem timestamps are ignored for resume selection.
- Simplified editor resume semantics: OPEN now loads the highest existing stage checkpoint as the last explicitly saved editor point. Production is used only when no checkpoint history exists yet; transient `wizard_state.json` validity never selects the resume head.
- REIMPORT/RELOAD/ordinary edits mutate only the current RAM working copy. They do not overwrite or delete saved checkpoints, so closing without a new stage checkpoint discards the temporary branch and the next OPEN resumes the same saved point.
- `COMPLETE STAGE + CHECKPOINT` is the only operation that advances linear editor history. After the new stage package + `editor_state.json` are written, every later-stage checkpoint directory is removed; saving an earlier stage intentionally discards its former descendants.
- Explicit RESTORE remains non-destructive. It can inspect/return to any still-existing stage point; later checkpoints are removed only if the restored/edited stage is subsequently saved as a new checkpoint. Startup also selects the resumed stage in the wizard UI.
...
- Kept the 0.10.23 editor/application terminals intact while replacing only the heavy geometry transport. Small commands and metadata remain JSON control-plane messages; vertex/index/normal/material/smoothing/edge arrays now travel in versioned `ELWIR001` binary WebSocket frames.
- Full asset publication is now a small `asset_binary_begin` descriptor followed by one binary frame per resident LOD. The browser transport adapter reassembles the same legacy `asset` object and only then calls the existing full-asset handler, so `acceptAssetState(..., true)` / `rebuildScene()` semantics are unchanged.
- On-demand LOD publication uses the same rule: `lod_payload_binary_begin` + one binary frame is decoded back into the same legacy `lod_payload` terminal before the existing viewport handler runs. Geometry ids/counts are validated before publication to prevent metadata/binary mismatches from producing a partial scene.
- Targeted `load_lod` / source-variant refresh no longer retransmit unchanged resident LOD arrays. They still terminate in the **same legacy full `asset` handler**: the control descriptor marks a transport delta, the browser reuses already-resident payloads for unchanged LODs, applies binary frames only for changed LODs, validates identity/counts, and only then invokes the untouched `asset` terminal. Operations that replace the complete working set still send a self-contained full binary snapshot.
- Browser geometry arrays stay as zero-copy typed views over the received `ArrayBuffer` (`Float32Array`, `Uint32Array`, `Int32Array`) until existing editor operations explicitly replace/clone them.
- `.elmesh` v4/v2 payloads are now read once into a contiguous byte buffer and parsed through the existing Reader API in memory-cursor mode. The on-disk v4 layout and all CanonicalMeshBuilder/libigl/Embree/LOD/identity semantics are unchanged.
