...
## Model Asset Editor — canonical mesh preparation / LOD handoff 0.10.17
...
- `[x]` Целевые authoring-типы геометрии: `ClosedVolume`, `ThinOneSided`, `ThinTwoSided`, `BreachedVolume`; `Invalid` должен оставаться редким случаем, который автоматическая подготовка действительно не может нормализовать.
...
- `[x]` 0.10.16 production acceptance на Orbital Station пройден: offline editor использует libigl `split_nonmanifold` + Embree raycast orientation; для `station_habitat_s3` после PREPARE `nonmanifold_edges=0`, `winding_conflicts=0`, `inward_closed=0`, реальные отверстия не закрываются.
- `[~]` 0.10.17 исправляет handoff в LODS: технически canonical LOD0 разрешает `ANALYZE LOD0` и LODS checkpoint без преждевременного требования `ClosedVolume / ThinTwoSided / BreachedVolume`; эти решения переносятся в SURFACES. Editor artifacts нормализуются под `build/tools/model_asset_editor/`.
...
## Model Asset Editor — generated LOD authoring 0.10.18

- `[x]` LOD0 canonical preparation and libigl/Embree orientation are production editor paths.
- `[x]` LOD analysis is no longer blocked by SURFACES classification.
- `[x]` LOD generation now covers the complete LOD0 geometry pool, including additional/replacement meshes with no RenderNode usage.
- `[x]` The author may preview individual main/replacement meshes and choose which generated LOD levels are applied.
- `[x]` Selected generated levels replace existing slots or create new contiguous RenderLods; LOD0 remains authoritative and unchanged.
- `[x]` Generated LODs preserve stable replacement identities and can be saved as the full LODS checkpoint for the GEOMETRY stage.
- `[>]` Next LOD algorithm step: consolidation/simplification of the surviving large structures after disconnected-detail culling. Do not make detail culling more destructive merely to reach a lower triangle budget.

### Model Asset Editor 0.10.20

GEOMETRY workspace restored after the LOD-generator work: automatic stage entry resets transient LOD preview state, active LOD selection is first, main/replacement meshes have explicit preview, existing instance/radial-array tools are surfaced in-stage, rigid-fit consolidation remains available, and replacement compatibility/preview is visible before GEOMETRY checkpointing.


### Model Asset Editor 0.10.21 — SURFACES

- `[x]` SOURCE, LODS and GEOMETRY are accepted functional stages; GEOMETRY usability cleanup is backlog rather than a blocker.
- `[x]` SURFACES is a real per-RenderLod wizard stage with its own checkpoint. It owns surface intent and material contract, and does not mutate topology, transforms, instance sharing or replacement relationships.
- `[x]` Production surface intents are `ClosedVolume`, `ThinOneSided`, `ThinTwoSided`, `BreachedVolume`; unresolved open used/replacement geometry blocks only the SURFACES checkpoint.
- `[x]` Material usage is audited per triangle. Invalid material indices or unassigned triangles block completion; a conservative tool can fill only unassigned triangles without rewriting existing assignments.
- `[x]` Shared MaterialDefinition properties are editable: stable id, base RGBA, emissive color/strength, metallic, roughness and base/emissive texture references. Material `twoSided` is no longer an ordinary authoring authority; geometry surface intent owns sidedness.
- `[x]` SURFACES viewport previews real material groups and front/two-sided behavior. Material/intent edits invalidate SURFACES and downstream stages, not LODS/GEOMETRY.
- `[>]` Next wizard stage: SEMANTICS. Texture import/bake/UV painting, per-face material painting and GEOMETRY UX cleanup remain deferred unless they become blockers.

### Model Asset Editor 0.10.22 — explicit surface analysis / cross-LOD intent

- `[x]` Opening SURFACES is side-effect free: topology/surface audit starts only from the explicit ANALYZE SURFACES button; simply revisiting the tab does not run it again.
- `[x]` Surface intent can be propagated with a default-on checkbox to the same stable base/replacement visual family across all RenderLods; transient `G#` and coincidental geometry indices are never used as cross-LOD identity.
- `[x]` Bulk propagation writes/publishes once and performs the expensive model preflight once after the batch instead of once per LOD.
- `[x]` `ClosedVolume`, `BreachedVolume`, `ThinOneSided` => FrontSide/culling ON; `ThinTwoSided` => DoubleSide/culling OFF. Surface intent is the sidedness authority.
- `[x]` SURFACES analysis is explicitly launched and cached across harmless tab revisits; upstream SOURCE/LODS/GEOMETRY metadata changes invalidate the cached analysis.
### Model Asset Editor 0.10.23 — executable-owned UI package isolation

- `[x]` Удалён архитектурный universal-pack contract: `HtmlUiServer` больше не ищет `elite_ui.pak` сам. Конкретный executable передаёт собственный resource-pack path.
- `[x]` `EliteGame` владеет `elite_game_ui.pak`; `EliteAssetEditor` владеет `model_asset_editor_ui.pak`. Game pack исключает `model_asset_editor.html`, editor pack включает только editor page и нужные Three.js modules.
