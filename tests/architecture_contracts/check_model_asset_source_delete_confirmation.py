...
for token in (
    ...
    if token not in session:
        raise AssertionError(f"schema-15 missing state not persisted/exposed: {token!r}")
...
if 'ModelAssetEditorVersion = "0.10.63"' not in text("tools/model_asset_editor/EditorVersion.h"):
    raise AssertionError("editor version is not 0.10.63")

print("[PASS] model asset editor v0.10.63 stage certification / confirmed SOURCE deletion")
