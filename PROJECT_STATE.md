...
8. `[~]` **P0.5 — canonical build/runtime layout и cleanup:** реализация подготовлена: runtime paths фиксированы как `build/EliteGame.exe` и `build/headless_server/EliteServer.exe`; обычный client configure больше не строит server по умолчанию; process/runtime acceptance использует ровно эти же binaries; compile-only test scratch перенесён под `build/tests/<suite>/`, логи — под `build/test-logs/`; `build_mingw64.sh` и ready harness удаляют известные legacy build directories и запрещают дубли `EliteGame.exe`/`EliteServer.exe`. Осталось пройти полный MinGW ready gate и ручной reconnect/two-client runtime acceptance на этой схеме, после чего P0.5 можно закрыть.
...
13. **Немедленный operational block — canonical build/runtime layout + cleanup.** Скриптовая схема уже сведена к единственным runtime paths `build/EliteGame.exe` и `build/headless_server/EliteServer.exe`; старые `ready_headless_server`/`network_process_server` и top-level `*_tests` объявлены legacy и автоматически удаляются, а test scratch живёт только под `build/tests/`. Следующий gate — полный MinGW regression + ручной reconnect/two-client acceptance именно на этих канонических binaries.
