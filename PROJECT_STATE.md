...
- `[~]` **Presentation atomicity / UI-map stability:** ручная трассировка выявила три разных источника артефактов, а не один CSS-баг: GLFW HWND создавался видимым до первого определённого framebuffer; WebView2 child оставался физически видимым во время cross-document navigation и мог показать белый controller/default-background frame до первого HTML paint; SystemMap panel выбрасывался при каждом закрытии карты, поэтому F9-F12 заново ждали HTML/CSS/fonts/layout перед началом перехода. Теперь top-level окно создаётся hidden и показывается только после dark bootstrap frame, WebView navigation всегда native-hidden и destination появляется только после `*_prepared` handshake, а target-agnostic SystemMap panel prewarm/reuse держит DOM/font layer прогретым в gameplay. Main-menu home route ограничен более компактной стабильной shell-height и центрируется по вертикали. F9-F12 сохраняют frame-owned outgoing/incoming barrier, но presentation fade сокращён до ~0.20 s, map mode-change до ~0.24 s. Требуется полный MinGW gate + ручная проверка startup white-flash, first/repeated F9-F12 latency, resize и service transitions перед переводом `[~] -> [x]`.
...
- `[~]` **ESC/session-menu client shell:** Local и Multiplayer физически разделены на `local_session_menu.html` и `multiplayer_session_menu.html`, поэтому локальные Save/Load/Main не могут смешаться с сетевыми Disconnect/Sign-out. New Game/Settings не дублируются. Save/Load и explicit Sign out остаются fail-closed до соответствующих backend slices. ESC использует dual-path input: Win32 edge latch признаёт WebView2 child/helper HWND частью foreground client, а видимая WebView дополнительно forward'ит Escape в тот же idempotent native command. Переходы session/service документов входят в общий acknowledged presentation transaction; ручной focus/rapid-toggle acceptance остаётся обязательным до закрытия пункта.
...
3. `[~]` **UI/auth client platform + service shell — финальный client-side polish перед M8E.3.** Binary `elite_ui.pak`, global Noto/license registry, reusable `EliteUiKit`, `ClientPreferencesStore` и service/session navigation готовы. Service WebView использует всю площадь окна, geometry/fonts масштабируются по высоте при min window 800×600; Local New Game спрашивает локальное Unicode-имя без moderation; remote `SIGN IN` единственный и сначала пробует remembered device, затем маршрутизирует к password fallback; remote loading можно отменить. `ACCOUNT` относится только к Multiplayer shell и появляется там после успешной auth. Текущая ручная acceptance-задача — подтвердить focus-independent ESC и единый fade-out/fade-in для всех service screen/document transitions. Password/recovery/sign-out backend по-прежнему ждёт M8E.3b.
