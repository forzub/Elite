...
- Every graphical client owns a process-local WebUI endpoint. The first `EliteGame` process acquires a cross-process lease and binds the canonical Debug Control port `localhost:8090`; additional clients continue to request port `0` and receive isolated OS-assigned endpoints. WebView2 also uses a process-local user-data directory, so clients never share UI/browser runtime state. The existing best-effort `localhost:8090` compatibility redirect remains useful when the primary process exits while another dynamic client is still alive. `Application` keeps printing the exact direct Debug Control URL for every process.
