...
6. `[~]` Runtime scaling: Stage 4A/4B уже применяют `Active / Prewarm / Coarse` к реальной стоимости materialized ships. AI/service/maintenance cadence снижена, а дорогой motion-control solver отделён от дешёвой fixed-step kinematic propagation. Multiplayer session/client foundation M1–M6 готов: per-session ship interest отделён от simulation activation, а protocol/client history уже различают full presence и sparse retain/update/remove. **Следующий шаг — реально включить per-entity sparse cadence**, сохранив full bootstrap/re-entry hydration.

### Multiplayer / session authority foundation

**Статус: `[~]` Stage M6 завершён: два настоящих `GameClient` работают на одном authoritative runtime, а сервер уже строит отдельный ship replication-interest plan для каждой session. Protocol/client state различают `FullAuthoritativeSet` и `SparseRetainMissing`, поэтому omission больше не обязан означать destruction. Следующий этап — включить реальный sparse/cadenced ship publication, затем настоящий network transport.**
...
- `[~]` Multiplayer contract теперь покрывает server sessions, multi-transport routing, client local-vs-remote identity, session-derived navigation, two-`GameClient` acceptance и per-session replication-interest/retention foundation. Следующий этап — реально применять interest cadence к ship payload, сохраняя full bootstrap/re-entry hydration.
- `[ ]` Включить sparse/delta ship replication: Controlled/Tactical — normal cadence, Nearby/Coarse — реже, `None` — вне current transport set; временное отсутствие entity в пакете означает retain, а lifecycle/interest exit идёт explicit remove. Перед фактическим скрытием payload server interest должен пересекаться с authoritative world-knowledge/visibility policy, чтобы network interest не стал утечкой скрытых объектов.
...
2. `[~]` **Multiplayer session/player authority foundation** — Stage M1–M6 готовы: session authority, multi-transport fan-out, local-vs-remote identity, session-derived navigation, two-`GameClient` acceptance и per-session replication-interest/retain-remove semantics. Далее — фактический sparse cadence и network transport.
...
6. Текущий следующий технический шаг — **включить фактический sparse ship publication поверх Stage M6**: per-session interest plan и retain/update/remove semantics уже готовы, но production snapshots пока сознательно остаются `FullAuthoritativeSet`. Нужно добавить full bootstrap/re-entry hydration и per-entity cadence без смешения network interest с gameplay visibility. После этого — network transport и возврат к `Scheduled <-> Coarse <-> Prewarm <-> Active` materialization/collapse.
