...
function finalAssemblyValidationHtml(model,text,fragments){
 const rows=(model.rows||[]).map(x=>`<div class="sourceInventoryRow"><span>${escapeMarkup(x.stage)}</span><span class="${x.passed?'ok':'bad'}">${x.passed?text.rowPass:text.rowFail}</span><span style="grid-column:3/6">${escapeMarkup(x.message)}</span></div>`).join('');
